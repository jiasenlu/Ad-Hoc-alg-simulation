   ...
   #-------------------------------------------------------------------     
    def set_PktRemain(self, Pkt):
        if (Pkt.Src, Pkt.Id) not in self.PktRemain:
            self.PktRemain[Pkt.Src, Pkt.Id] = self.PerPkt
   #--------------------------------------------------------------------
    def set_Timeout(self, i, j):
   
        if (i, j) not in self.Timeout:
            self.Timeout[i, j] = self.Pertimeout
...
def gen_nodes(Length = 1000, Width = 1000, Number = 100):
    ...
    Nodes[0].Location = [9,7]
    Nodes[1].Location = [0,6]
    Nodes[2].Location = [0,2]
    Nodes[3].Location = [8,0]
    Nodes[4].Location = [9,4]
    Nodes[5].Location = [0,6]
    Nodes[6].Location = [3,8]
    Nodes[7].Location = [9,3]
    Nodes[8].Location = [2,4]
    Nodes[9].Location = [1,2]
                        ...
                        if Pkt.Dst == i:
                            ...
                            if Pkt.Flag == 2:
                                if [Pkt.Src, Pkt.Id, Pkt.Num] not in Nodes[i].Recieve_Pkt:
                                    ...
                                    Nodes[i].Recieve_TTL.append([Pkt
                              #  print Pkt.Id
                             #   print Pkt.Num
                        j -= 1
                    j += 1
...
def forward_Pkt(Nodes, Adj,RQp, Pkt, Nbrs, i):
    ...
    if Flag == 0:
        for Nbr in Nbrs:
            if len(RQp[Nbr])<=Nodes[Nbr].Buffer and [Pkt.Id, Pkt.Num, Pkt.Src] not in Nodes[Nbr].Packet_State:
                if Pkt.RecoveryHop > 0:
                        
                    NPkt =  Packet(Pkt.Id, Pkt.Src, Pkt.Dst, 1, Pkt.TTL - 1, Pkt.Num)
                    Nodes[Nbr].update_Packet_State(Pkt.Id, Pkt.Num, Pkt.Src)
                    NPkt.RecoveryHop = Pkt.RecoveryHop - 1
                    RQp[Nbr].insert(0,NPkt)
                    print 'Pakcet [%d, %d, %d] from %d to %d with Rhop %d' %(Pkt.Src, Pkt.Id, Pkt.Num, i, k, NPkt.RecoveryHop)
