...
class RREQ:
    ...
    def update_Border(self, Nodes):
        self.Border = [Nodes[self.Src].Location, Nodes[self.Dst].Location]
...
def update_Nodes(Nodes):
    '''update the iPakcet [53, 1, 3] from Node 0 to Node 4
nformation of Nodes, include the NLocation, Grid ect.
    '''
                    ...
                    if Pkt.Dst == i or Pkt.TTL <= 0: 
                        ...
                        if Pkt.Dst == i:
                            ...
                            # suppose it's RREQ, then it should foward the RREP
                                ...
                                RQ[i].insert(0,NPkt)
                                print 'Node %d generate RREP [%d, %d] to Node %d' %(i, Pkt.Dst, Pkt.Id, Pkt.Src)
                            if Pkt.Flag == 1:
                            # it's RREP, the node set the PktRemain to forward the packet,
                                Nodes[i].set_PktRemain(Pkt)
                                
                                print 'Node %d recieve RREP [%d, %d] successfuly' %(i, Pkt.Id, Pkt.Src)

                                Nodes[i].del_Timeout(Pkt.Id, Pkt.Dst)
                            if Pkt.Flag == 2:
                                if [Pkt.Src, Pkt.Id, Pkt.Num] not in Nodes[i].Recieve_Pkt:
                                    print 'Node %d recieve Packet [%d, %d, %d] Successfully' %(i, Pkt.Src, Pkt.Id, Pkt.Num)
...
def gen_RREQ(Nodes, RQ, RQNum, InitTTL):
    ...
    for i in xrange(RQNum):
        ...
        RQ[Src].insert(0, Pkt)
        print 'Node %d generate RREQ %d to Destination %d' %(Src, Pkt.Id, Pkt.Dst)

    return Nodes, RQ
Node 1 generate Packet 1 Num 1 to Destination 5
        ...
        if Nodes[i].PktRemain != {}:
            ...
            for j in xrange(len(Temp)):
                if Temp[j][1] > 0:
                    ...
                    print 'Node %d generate Packet %d Num %d to Destination %d' %(i, Temp[j][0][1], 5-Nodes[i].PktRemain[Temp[j][0]], Temp[j][0][0])
        ...
        # if there is destination information in Routingtable
            ...
            if k in Nbrs:
                if len(RQp[k]) <= Nodes[k].Buffer:
                    ...
                    print 'RREP [%d, %d] from Node %d to  Node %d' %(Pkt.Src, Pkt.Id, i, k)
            else:
                for Nbr in Nbrs:
...
def forward_Pkt(Nodes, Adj, RQp, Pkt, Nbrs, i):
    ...
    for j in xrange(0, len(Nodes[i].RREP_RoutingTable)):
        if Pkt.Dst == Nodes[i].RREP_RoutingTable[j][0]:
            ...
            if k in Nbrs and len(RQp[k]) <= Nodes[k].Buffer:
                ...
                RQp[k].insert(0,NPkt)
                Flag = 1
                print 'Pakcet [%d, %d, %d] from Node %d to Node %d' %(Pkt.Src, Pkt.Id, Pkt.Num, j, k)
    ...
    if Flag == 0:
        if Pkt.RouteRecovery > 0:
            ...
            if Pkt.RouteRecovery == 4:
                for Nbr in Nbrs:
                    if len(RQp[Nbr])<=Nodes[Nbr].Buffer:
                        ...
                        RQp[Nbr].insert(0,NPkt)
                        print 'Generate RRER [%d, %d] at Node %d' %(Pkt.Src, Pkt.Id, Nbr)
            ...
            print 'Drop Packet [%d, %d] at Node %d' %(Pkt.Src, Pkt.Id, j)
...
def determine_Border(Nodes, Pkt, i):
    '''
    determine whether the Node i is in RREQ transimision border. if yes return 1 else return 0.
    '''
    [X,Y] = Nodes[i].Location
    X_New = Pkt.Border[1][0]
    Y_New = Pkt.Border[1][1] 
#    print Pkt.Border
#    print Nodes[i].Location
    if (Pkt.Border[0][0] <= X <= X_New or X_New <= X <= Pkt.Border[0][0] or
        Pkt.Border[0][1] <= Y <= Y_New or Y_New <= Y <= Pkt.Border[0][1]):
        return True
    else:
        return False
