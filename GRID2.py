...
class node:
    ...
    def copy_RoutingTable(self, Node):
        ...
        self.RREP_State = copy.deepcopy(Node.RREP_State)
        ...
        self.RREP_Table_Time = copy.deepcopy(Node.RREP_Table_Time)
        # the routingtable contains the destination, nextcount and the trail.
        self.RREP_recieve = copy.deepcopy(Node.RREP_recieve)
        self.PktRemain = copy.deepcopy(Node.PktRemain)
        # the PktRemain contains the destination, RREQ.Id and the remain to forward packet.
...
def prepare(Nodes, RQ, TxRng, InitTTL, Iter):
    ...
    Nodes = elect_Gate(Nodes, Iter)
    Nodes, RQ = check_RQ(Nodes, RQ)
    RQp = prepare_RQp(RQ)
    return Nodes, Adj, RQ, RQp   

def elect_Gate(Nodes, Iter):
    for i in xrange(0, len(Nodes)):
        Nodes[i].GateTimer += 1
        Nodes[i].BidTimer += 1
        if Iter == 1:
            ...
            for j in xrange(0, len(Nodes)):
                if Nodes[j].Grid == Nodes[i].Grid:
                    ...
                    # gate node's Id.
                        ...
                        X = dist(Nodes[i].Location, [Nodes[i].Grid_length * 
                        (Nodes[i].Grid[0]+0.5), Nodes[i].Grid_length * (Nodes[i].Grid[1]+0.5)])      
                        Y = dist(Nodes[j].Location, [Nodes[j].Grid_length * 
...
def broadcast_RETIRE(Nodes, i):
    ...
    if Temp_node.Grid != Nodes[i].Grid:
        for j in xrange(0, len(Nodes)):
    ...
    if Flag == 0:
        for Nbr in Nbrs:
            if [Pkt.Id, Pkt.Src] not in Nodes[Nbr].RREQ_State and len(RQp[i]) <= Nodes[i].Buffer :
                if Nodes[Nbr].GateNode == Nbr and determine_Border(Nodes, Pkt, Nbr):
                ...
                for k in xrange(0, len(Nodes)):
			        ...
			        # if there exist gate node in that grid.
                        if k in Nbrs and len(RQp[k]) <= Nodes[k].Buffer:
                            ...
                            RQp[k].insert(0, NPkt)
                            NFlag = 1
                            print 'RREP [%d, %d] from %d to %d' %(Pkt.Dst, Pkt.Id, i, k)
        if NFlag == 0:
...
def forward_Pkt(Nodes, Adj, RQp, Pkt, Nbrs, i):
    ...
    for Nbr in Nbrs:
        if Pkt.Dst == Nbr:
            ...
            RQp[Nbr].insert(0,NPkt)
            NFlag = 1
            print 'Pakcet [%d, %d, %d] from %d to %d' %(Pkt.Src, Pkt.Id, Pkt.Num, i, Nbr)
    if NFlag == 0:
        for j in xrange(0, len(Nodes[i].RREP_RoutingTable)):
            if Pkt.Dst == Nodes[i].RREP_RoutingTable[j][0]:
                print Nodes[i].RREP_RoutingTable[j][2]
                Next_Grid = list(Nodes[i].RREP_RoutingTable[j][2])
                ...
                for k in xrange(0, len(Nodes)):
                    if Nodes[k].Grid == Next_Grid and Nodes[k].GateNode == k:
                        if k in Nbrs and len(RQp[k]) <= Nodes[k].Buffer:
                            ...
                            RQp[k].insert(0,NPkt)
                            Flag = 1
                            print 'Pakcet [%d, %d, %d] from %d to %d' %(Pkt.Src, Pkt.Id, Pkt.Num, i, k)
        ...
        if Flag == 0:
            for Nbr in Nbrs:
                if Nodes[Nbr].GateNode == Nbr and len(RQp[Nbr])<=Nodes[Nbr].Buffer:
                    if Pkt.RouteRecovery > 0:
                        NPkt =  Packet(Pkt.Id, Pkt.Src, Pkt.Dst, 1, Pkt.TTL - 1, Pkt.Num)
                        NPkt.RouteRecovery = Pkt.RouteRecovery - 1
                        Nodes[Nbr].update_Packet_State(Pkt.Id, Pkt.Num, Pkt.Src)
                        RQp[Nbr].insert(0,NPkt)
                        print 'Pakcet [%d, %d, %d] from %d to %d with Rhop %d' %(Pkt.Src, Pkt.Id, Pkt.Num, i, Nbr, NPkt.RouteRecovery)
...
#    print Nodes[i].Location
    if ((Pkt.Border[0][0] <= X <= X_New + 5 or X_New - 5 <= X <= Pkt.Border[0][0]) and 
        (Pkt.Border[0][1] <= Y <= Y_New + 5 or Y_New - 5 <= Y <= Pkt.Border[0][1])):
        return True
    else:
        return False
